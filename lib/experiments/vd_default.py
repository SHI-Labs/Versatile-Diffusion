...
class eval_stage_i2t2i(eval_stage):
    ...
    def __call__(self, **paras):
        ...
        replicate = cfgv.replicate
        conditioning = cfgv.conditioning*replicate
        conditioning_local = conditioning[LRANK : len(conditioning) : LWSIZE]
        seed_increment = [i for i in range(len(conditioning))][LRANK : len(conditioning)*replicate : LWSIZE]

        prompt_temperature = cfguh().cfg.eval.get('prompt_temperature', 1.0)
        color_adj = cfguh().cfg.eval.get('color_adj', False)
        color_adj_keep_ratio = cfguh().cfg.eval.get('color_adj_keep_ratio', 0)
        sample_kwargs = copy.deepcopy(cfgv.sample)
        sample_kwargs.pop('mixed_ratio')
        sample_kwargs_dc = copy.deepcopy(cfgv.sample)
        sample_kwargs_dc.pop('n_samples')

        for (im, tfrom, tto), seedi in zip(conditioning_local, seed_increment):
            if fixed_seed is not None:
                np.random.seed(cfg.env.rnd_seed + seedi)
                torch.manual_seed(cfg.env.rnd_seed + seedi + 100)

            output_name = '{}_rm_{}_add_{}_seed{}'.format(
                osp.splitext(osp.basename(im))[0], 
                tfrom.strip().replace(' ', '-'), 
                tto.strip().replace(' ', '-'), 
                cfg.env.rnd_seed + seedi + 100, )

            c = tvtrans.ToTensor()(PIL.Image.open(im))
            c = c*2 - 1

            if with_ema:
                with netm.ema_scope():
                    zt, _ = self.sample(netm, sampler, c, 'text', 'vision', **sample_kwargs)
                    ztf = netm.optimus_encode([tfrom])
                    ztt = netm.optimus_encode([tto])

                    caption = netm.optimus_decode(zt, temperature=prompt_temperature)
                    zi, _ = self.sample_dc(
                        netm, sampler, (c, caption, tfrom, tto), **sample_kwargs_dc)
            else:
                zt, _ = self.sample(netm, sampler, c, 'text', 'vision', **sample_kwargs)
                ztf = netm.optimus_encode([tfrom])
                ztt = netm.optimus_encode([tto])
                ztf_norm = ztf / ztf.norm(dim=1)
                zt_proj_mag = torch.matmul(zt, ztf_norm[0])
                zt_perp = zt - zt_proj_mag[:, None] * ztf_norm
                zt_newd = zt_perp + ztt

                caption = netm.optimus_decode(zt_newd, temperature=prompt_temperature)
                zi, _ = self.sample_dc(
                    netm, sampler, (c, caption, tfrom, tto), **sample_kwargs_dc)

            self.save_text(caption, output_name, output_path, suffix=suffix)
            self.decode_and_save(
                netm, zi, 'image', output_path, output_name, suffix, 
                color_adj=color_adj, color_adj_to=im, 
                color_adj_keep_ratio = color_adj_keep_ratio)

        if eval_cnt is not None:
            print_log('Demo printed for {}'.format(eval_cnt))
        return {}

    def simple_change_low_rank(self, x, demean=True, q=20, niter=10, q_breakpoint=10, ratio=0.5, op_forward=True):
        if demean:
        ...
        if op_forward:
            s[:, 0:q_breakpoint] = s[:, 0:q_breakpoint] * ratio
        else:
            s[:, q_breakpoint:] = s[:, q_breakpoint:] * ratio
    ...
    def get_vision_context(self, net, x):
        c = net.clip_encode_vision(x)
        u = net.clip_encode_vision(torch.zeros_like(x))
        c_global = c[:, 0:1]
        c_local  = c[:, 1: ]
        c_local_new = self.simple_change_low_rank(
            c_local, q=10, q_breakpoint=3, demean=True, ratio=0, op_forward=False)
        # cimage = torch.cat([cimage_global, cimage_local_new], dim=1)
        c_new = c_local_new
        u_new = u[:, 1:]
        return c_new, u_new

    def get_prompt_context(self, net, cr, cf, ct):
        n_samples = len(cr)
        cr = net.clip_encode_text(cr)
        ct = net.clip_encode_text([ct])
        c = torch.cat([cr, ct.repeat(n_samples, 1, 1)], dim=1)
    ...
    @torch.no_grad()
    def sample_dc(
            self, net, sampler, context, image_output_dim, text_latent_dim,
            scale, ddim_steps, ddim_eta, mixed_ratio):
        c_img, c_ref, c_from, c_to = context
        n_samples = len(c_ref)
        c_img = c_img[None].repeat(n_samples, 1, 1, 1)

        cvision, uvision = self.get_vision_context(net, c_img)
        cprompt, uprompt = self.get_prompt_context(net, c_ref, c_from, c_to)

        h, w = image_output_dim
        shape = [n_samples, 4, h//8, w//8]
        rv = sampler.sample_dc(
            steps=ddim_steps,
            shape=shape,
            first_conditioning=[uvision, cvision],
            second_conditioning=[uprompt, cprompt],
            unconditional_guidance_scale=scale,
            xtype='image', 
            first_ctype='vision',
            second_ctype='prompt',
    ...
    def decode_and_save(
            self, netm, z, xtype, path, name, suffix,
            color_adj=False, color_adj_to=None, color_adj_keep_ratio=0):
        if xtype == 'image':
            x = netm.autokl_decode(z)
            if color_adj:
                x_adj = []
                for xi in x:
                    color_adj_f = color_adjust(ref_from=(xi+1)/2, ref_to=color_adj_to)
                    xi_adj = color_adj_f((xi+1)/2, keep=color_adj_keep_ratio)
                    x_adj.append(xi_adj)
                self.save_images(x_adj, name, path, suffix=suffix+'_coloradj')
            self.save_images(x, name, path, suffix=suffix)
        elif xtype == 'text':
            temperature = cfguh().cfg.eval.get('prompt_temperature', 1.0)
            x = netm.optimus_decode(z, temperature=temperature)
            self.save_text(x, name, path, suffix=suffix)
